...
class JsonProfile extends CI_Controller
{
    ...
    public function register()
    {
        ...
        $this->form_validation->set_rules('deviceId', 'deviceId', 'trim|required|min_length[2]|is_unique[tg_client.imei]', array('is_unique' => 'این دستگاه قبلا در سیستم ثبت شده است / وارد حساب شوید'));
        if ($this->form_validation->run() === false) {
            ...
            if($this->user_model->userExist($PhoneNumber)){
                echo json_encode([
                    'data'=>[],
                    'ErrorMessage'=>'این شماره قبلا ثبت شده'
                ]);
                return;
            }
            if($this->user_model->RegisterUser($PhoneNumber)){
                echo json_encode([
                    'data'=>$this->user_model->getUser($PhoneNumber),
                    'ErrorMessage'=>''
                ]);
                return;
            }else{
...
این کد 4 رقمی را در برنامه وارد کنید : <strong>' . $hash . '</strong><br/>
 در صورت وجود هرگونه سوال میتوانید با ایمیل زیر با ما در تماس باشید : <br/>info@bahar.persianstory.ir
                ...
                if ($this->email->send()) {
                    ...
                    echo "مشکل در عملیت نوسازی کلمه عبور";
                }
            } else {
                echo "این ایمیل در سیستم موجود نیست";
            }
